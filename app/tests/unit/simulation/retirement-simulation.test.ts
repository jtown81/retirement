...
import { unifiedRetirementSimulation } from '../../../src/modules/simulation';
...
describe('unifiedRetirementSimulation', () => {
  it('returns correct number of years', () => {
    const result = unifiedRetirementSimulation(makeConfig({ retirementAge: 60, endAge: 95 }));
    expect(result.years).toHaveLength(36); // ages 60-95 inclusive
  });

  it('first year starts at retirement age', () => {
    const result = unifiedRetirementSimulation(makeConfig());
    expect(result.years[0].age).toBe(60);
  });

  it('last year ends at end age', () => {
    const result = unifiedRetirementSimulation(makeConfig({ endAge: 95 }));
    expect(result.years[result.years.length - 1].age).toBe(95);
  });

  it('FERS annuity grows by COLA each year', () => {
    const result = unifiedRetirementSimulation(makeConfig({ colaRate: 0.02 }));
  ...
  it('FERS supplement ends at age 62', () => {
    const result = unifiedRetirementSimulation(makeConfig({ retirementAge: 60 }));
  ...
  it('Social Security starts at age 62', () => {
    const result = unifiedRetirementSimulation(makeConfig({ retirementAge: 60, ssMonthlyAt62: 1800 }));
  ...
  it('uses GoGo/GoSlow/NoGo smile curve multipliers', () => {
    ...
    const result = unifiedRetirementSimulation(config);
  ...
  it('tracks Traditional and Roth balances separately', () => {
    const result = unifiedRetirementSimulation(makeConfig({ traditionalPct: 0.70 }));
    const yr0 = result.years[0];
  ...
  it('RMD is computed at age 73+', () => {
    const result = unifiedRetirementSimulation(makeConfig({ retirementAge: 60, endAge: 80 }));
    // age 73 = index 13
